...
for run in tqdm(runs):
    ...
    for artifact in run.logged_artifacts():
        if artifact.name[: len(artifact_name)] == artifact_name:
            run_name_to_artifacts[run.name].append((run.repeat_number, artifact))
    ...
    for repeat_number, artifact in artifact_tuples:
        ...
        if not os.path.exists(complete_path):
            ...
            if repeat_number is None:
                repeat_path = os.path.join(complete_path, "_None")
            else:
                repeat_path = os.path.join(complete_path, "_" + str(repeat_number))
            if not os.path.exists(repeat_path):
                os.mkdir(repeat_path)
                artifact.download(complete_path)
            else:
